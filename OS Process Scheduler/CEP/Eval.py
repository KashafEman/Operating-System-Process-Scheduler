...
def animation():
    ...
    if is_playing:
        ...
        if count == len(frames)-100:
            ...
            def simulate():
                app.destroy()
                subprocess.run([sys.executable, 'CEP\\agenda.py'])
            ...
            # Cleanup function to destroy the pie chart canvas when the app is closed
            def on_close():
                if showAnimation is not None:
                    app.after_cancel(showAnimation)  # Cancel the scheduled callback
                canvas3.get_tk_widget().destroy()
                app.destroy()

            app.protocol("WM_DELETE_WINDOW", on_close)  # Handle window close event
