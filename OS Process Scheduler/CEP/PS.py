...
from algos import Process, read_processes_from_dataframe, priority_scheduling_preemptive, evaluate_performance
        ...
        def animate():
            ...
            if tab_isplaying and tab_count < len(frames):
                ...
                if tab_count == len(frames)-1:
                    ...
                    def next():
                        app.destroy()
                        subprocess.run([sys.executable, 'CEP\MQS.py'])
                    ...
                    def back():
                        app.destroy()
                        subprocess.run([sys.executable, 'CEP\RR.py'])
...
scheduled_processes1 = priority_scheduling_preemptive(processes)

avg_turnaround_time, avg_waiting_time = evaluate_performance(scheduled_processes1)
insert_performance_data("PS", avg_turnaround_time, avg_waiting_time)
...
tabs = []
tab_names = ["Pre-emptive Priority Scheduling"]  # Add more tab names here
gif_paths = [
                    "CEP\PPS.gif"
